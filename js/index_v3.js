// adjust indention for readability
...
var search = function (t, prev_result) {
    ...
    document.querySelector("#fileNameDisplay").innerText = "playback source";
    ...
    document.querySelector("#searchBtn span").classList.add("glyphicon-repeat");
    ...
    if (document.querySelector("#search2Btn span")) {
        ...
        document.querySelector("#search2Btn span").classList.add("glyphicon-repeat");
    ...
    if (trial === 0) {
        document.querySelector("#results").innerHTML = "<div id=\"status\" class=\"status\">Submitting image for searching...</div>";
    }
    ...
    document.querySelector("#seasonSelector").disabled = true; /* add season selector to disable aswell */
    ...
    xhr.onreadystatechange = function () {
        document.querySelector("#searchBtn span").classList.remove("glyphicon-repeat");
        ...
        if (document.querySelector("#search2Btn span")) {
            document.querySelector("#search2Btn span").classList.remove("glyphicon-repeat");
        ...
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            document.querySelector("#seasonSelector").disabled = false; /* add season selector to disable aswell */
            ...
            document.querySelector("#results").innerHTML += "<div id=\"status\" class=\"status\">" + data.RawDocsCount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " images searched.</div>";
            if (prev_result) {
            ...
            if (data.docs.length > 0) {
                data.docs.forEach(function (entry, index) {
                    if (document.querySelector("#safeBtn .glyphicon").classList.contains("glyphicon-check") === false || entry.is_adult === false) {
                        ...
                        var thumbnailImage = (trial >= 2 || parseFloat(data.docs[0].diff) < 10) && index < 5 ? "<div class=\"thumb\"><img src=\"" + thumbnailLink + "\"></div>" : ""; /* removed fix height due to overflowing */
                ...
                keepSearchDiv.innerHTML = "<button id=\"search2Btn\" type=\"button\" class=\"btn btn--search\"><span class=\"glyphicon glyphicon-search\"></span> Keep Searching</button>";
                ...
                document.querySelector("#results").innerHTML = "<div id=\"status\" class=\"status\">No result</div>";
            }
        } else if (xhr.status === 429) {
            document.querySelector("#loading").classList.add("hidden"); /* reset loading ripple as well */
            document.querySelector("#loader").classList.remove("ripple");
            document.querySelector("#results").innerHTML = "<div id=\"status\" class=\"status text--danger\">" + xhr.responseText + "</div>";
        } else {
            /* reset loading ripple as well */
            document.querySelector("#loading").classList.add("hidden");
            document.querySelector("#loader").classList.remove("ripple");
            document.querySelector("#results").innerHTML = "<div id=\"status\" class=\"status text--danger\"><strong>The connection to the server failed.</strong> <a href=\"https://github.com/soruly/trace.moe/issues\">Open Issue</a></div>";
        }
        document.querySelector("#searchBtn").disabled = false;
        if (document.querySelector("#search2Btn span")) {
            document.querySelector("#search2Btn").disabled = false;
...
var playfile = function () {
    ...
    xhr.onreadystatechange = function () {
        ...
        var width = parseFloat(t) / parseFloat(xhr.responseText) * 640 - 6; // replaced to width

        document.querySelector("#progressBarControl").style.visibility = "visible";
        document.querySelector("#progressBarControl").style.width = width + "px"; // replaced to width
    };
    xhr.send();
...
var prepareSearchImage = function () {
    ...
    document.querySelector("#results").innerHTML = "<div id=\"status\" class=\"status\">Press 'Search' button to begin searching.</div>";
...
var handleFileSelect = function (evt) {
    ...
    if (file) {
        document.querySelector("#loading").classList.add("hidden"); /* reset loading ripple as well */
        document.querySelector("#loader").classList.remove("ripple");
        resetInfo(); /* reset info if not an image (known error) */
        document.querySelector("#results").innerHTML = "<div id=\"status\" class=\"status\">Reading File...</div>";
        if (file.type.match("image.*")) {
            ...
            document.querySelector("#results").innerHTML = "<div id=\"status\" class=\"status text--danger\">There was a problem uploading your image file. Please try again.</div>";
            return false;
        }
    }
};
