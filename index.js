            ...
            $.each(data.docs, function(key, entry){
                if(safeSearch == false || entry.season != 'Sukebei'){
                    ...
                    if(formatTime(entry.from) == formatTime(entry.to))
                        result.innerHTML = '<a href="#"><span class="title">'+entry.title+'</span><br><span class="ep">EP#'+zeroPad(entry.episode,2)+'</span> <span class="time">'+formatTime(entry.from)+'</span> <span class="similarity">~'+similarity+'%</span><br><span class="file">'+entry.file+'</span></a>';
                    else
                        result.innerHTML = '<a href="#"><span class="title">'+entry.title+'</span><br><span class="ep">EP#'+zeroPad(entry.episode,2)+'</span> <span class="time">'+formatTime(entry.from)+'-'+formatTime(entry.to)+'</span> <span class="similarity">~'+similarity+'%</span><br><span class="file">'+entry.file+'</span></a>';
